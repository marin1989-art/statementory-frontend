...
const IMAGES = [
  { src: '/error-404-back.png',    alt: 'Error 404 Diese Regierung — Rückseite mit Motiv' },
  { src: '/shirt-black-front.png', alt: 'Error 404 Diese Regierung — Vorderseite' },
];
...
export default function Error404Page() {
  ...
  const handleAddToCart = () => {
    ...
    setSizeError('');
    addItem({ sync_variant_id: VARIANT_IDS[selectedSize], productName: 'Error 404 — Diese Regierung', size: selectedSize, price_cents: 2900, quantity: 1, image: '/error-404-back.png' });
    setAdded(true);
  ...
  return (
    <main className="min-h-screen bg-black text-white">
      ...
      <div className="min-h-screen grid grid-cols-1 md:grid-cols-2">
        <ProductImagePanel images={IMAGES} />
        <div className="flex flex-col justify-center px-4 pt-8 pb-12 md:px-12 md:pt-32 md:pb-24 max-w-xl">
          ...
          <h1 className="text-3xl md:text-5xl font-black uppercase tracking-tight leading-[0.9] mb-8">
          ...
          <button onClick={handleAddToCart}
            ...
            {added ? '✓ Im Einkaufswagen' : 'In den Einkaufswagen — 29,00 €'}
          </button>
          {added && (
          ...
          <SizeChart measurements={GILDAN_MEASUREMENTS} />
          <ShippingInfo />
          <div className="w-8 border-t border-white/10 mt-12 mb-10" />
          <ProductDetails variant="gildan5000" />
        </div>
      </div>
