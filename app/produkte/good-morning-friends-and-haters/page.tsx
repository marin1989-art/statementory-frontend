...
export default function GoodMorningPage() {
  ...
  const handleAddToCart = () => {
    ...
    setSizeError('');
    addItem({ sync_variant_id: VARIANT_IDS[selectedSize], productName: 'Good Morning, Friends And Haters', size: selectedSize, price_cents: 3900, quantity: 1, image: '/goodmorning.png' });
    setAdded(true);
  ...
  return (
    <main className="min-h-screen bg-black text-white">
      ...
      <div className="min-h-screen grid grid-cols-1 md:grid-cols-2">
        <div className="md:sticky md:top-0 md:h-screen flex items-center justify-center bg-zinc-100 border-r border-white/5 p-6 pt-20 md:p-12 md:pt-24">
          <div className="w-full max-w-md aspect-square relative overflow-hidden">
            <Image src="/goodmorning.png" alt="Good Morning, Friends And Haters — Streetwear T-Shirt Mockup" fill className="object-contain" priority sizes="(max-width: 768px) 100vw, 50vw" />
          </div>
        </div>
        <div className="flex flex-col justify-center px-4 pt-8 pb-12 md:px-12 md:pt-32 md:pb-24 max-w-xl">
          ...
          <h1 className="text-4xl md:text-6xl font-black uppercase tracking-tight leading-[0.9] mb-8">
            <span className="block mb-1">Good Morning,</span>
            <span className="block text-transparent bg-clip-text bg-gradient-to-r from-white to-zinc-600">Friends And Haters.</span>
          </h1>
